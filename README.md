...
`npm run fetch-pool-price` reads the **Uniswap V2 pair** reserves (`getReserves`) and **appends** to **`public/pool-price.json`**: `{ "timestamp", "price", "pair" }`. **Price** = **fETH per 1 FT564** (spot mid-price from reserves; reciprocal of fs564 per fETH). The helper **`appendPoolPriceJson`** is exported from `scripts/fetch-pool-price.ts` for reuse.
